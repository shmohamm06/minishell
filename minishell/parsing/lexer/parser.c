...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/23 11:11:52 by shmohamm          #+#    #+#             */
/*   Updated: 2024/06/23 11:13:01 by shmohamm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_simple_cmds	*initialize_cmd(t_parser_tools *parser_tools)
{
	...
	return (ft_simple_cmdsnew(str, parser_tools->num_redirections,
...
int	handle_pipe_errors(t_tools *tools, t_tokens token)
{
	...
	if (!tools->lexer_list)
	{
		parser_error(tools, tools->lexer_list);
		return (EXIT_FAILURE);
	}
	return (EXIT_SUCCESS);
}
...
int	parser(t_tools *tools)
{
	...
	while (tools->lexer_list)
	{
		...
		if (!node)
		{
			parser_error(tools, parser_tools.lexer_list);
			return (EXIT_FAILURE);
		}
